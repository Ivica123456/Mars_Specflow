...
Scenario: [ 1User can successfully login to Mars Portal with valid credentials]
	...
	Examples: 
	| email                  | password |
	| ivica.cuncic@gmail.com | invalid |  
	| invalid@gmail.com      | mars123  |  

	Scenario Outline: [ 3User cannot successfully login to Mars Portal with empty credentials]
	Given [ User is on the Mars Portal login page]
	When [User enters empty '<email>' and '<password>' and click on login button] 
	Then [User should see an error message indicating that the login credentials are required]

	Examples: 
	| email                  | password |
	|                        |           |   
	| ivica.cuncic@gmail.com |           |   
	|                        | mars123  |   
